	...
	.setRun(async (client, interaction) => {
		...
		collector.on("collect", async (iter) => {
			...
			helpEmbed.addField(
				...
				"\n" +
				`[✨ Support Server](${ client.config.supportServer }) | [Issues](${ client.config.Issues }) | [Source](https://github.com/XSaitoKungX/Arsenal-3.0) | [Invite Me](https://discord.com/oauth2/authorize?client_id=${ client.config.clientId }&permissions=${ client.config.permissions }&scope=bot%20applications.commands)`,
			);
			await iter.update({
				embeds: [helpEmbed],
				components: [getButtons(pageNo)],
